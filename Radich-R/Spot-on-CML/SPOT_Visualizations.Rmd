...
```{r Data, split dbs/baseline}
data <- read.csv("SPOTOnCML_DATA_2018-05-30.csv")

baseline <- filter(data, redcap_event_name == "baseline_arm_1") # Country and site info
dbs <- filter(data, redcap_event_name == "dbs_arm_1") # Patient info
```


```{r Country raw}
dbs$country <- baseline$country

# Key-value pairs for countries and institutions
countries <- data.frame(Key = c(1:12),
                        Value = c("India","Uganda","Nigeria",
                                  "Phillipines","Tajikistan",
                                  "Cambodia","Timor Leste","Mongolia",
                                  "Nepal","Cote d\'Ivoire","Honduras",
                                  "Niger"),
                        stringsAsFactors = FALSE)
```

```{r Institution raw}
dbs$institution <- baseline$institution

institutions <- data.frame(Key = c(1:9),
                           Value = c("National Medical Center (Tajikistan)",
                                     "Uganda Cancer Institute",
                                     "University of Benin Teaching Hospital",
                                     "NHA Abuja",
                                     "Bairo Pite Clinic",
                                     "First Central Hospital (Mongolia)",
                                     "B.P. Koirala Memorial Hospital",
                                     "Chu de Yopougon",
                                     "Centro de Cancer Emma Romero de Callejas"))

```

```{r Clean factor levels}
# NA values are Niger, for now.  That may get replaced later
dbs$country[is.na(dbs$country)] <- 12
dbs$institution[is.na(dbs$institution)] <- 9

# Apply key-value pairs
dbs <- dbs %>% mutate(country = countries$Value[country]) %>%
                mutate(institution = institutions$Value[institution])
```




```{r Spot date over time}
## Spot date over time
dbs$spot_date <- as.Date(as.character(dbs$spot_date), format = "%m/%d/%Y")
dbs$yearMonth <- as.Date(as.Date(dbs$spot_date, format = "%m/%d/%Y"), format ="%m-%d-%Y")
dbs$yearMonth <- as.Date(gsub("-[0-9][0-9]$", "-01", as.character(dbs$yearMonth)))

byMonth <- dbs %>% group_by(yearMonth, country) %>%
  filter(!is.na(yearMonth)) %>% summarise(Samples = n()) %>%
  pad(group = "country") %>% fill_by_value(value = 0)
 
ggplot(byMonth, aes(x = yearMonth, y = Samples,color = factor(country))) +
  geom_point(size = 2, alpha = 0.7) + geom_line() +
  scale_x_date(breaks = byMonth$yearMonth,
               labels = format(as.Date(byMonth$yearMonth,
                                       format = "%m-%d-%Y"), "%b\n%Y"),
               limits = c(min(byMonth$yearMonth), max(byMonth$yearMonth))) +
...
```{r Transit / processing time}
# Finding transit time / processing time
dbs$date_received <- as.Date(as.character(dbs$date_received), format = "%m/%d/%Y")
dbs$test_date <- as.Date(as.character(dbs$test_date), format = "%m/%d/%Y")
dbs <- dbs %>% mutate(transitTime = date_received - spot_date,
                      testTime = test_date - date_received)

ggplot(dbs, aes(x = transitTime, fill = factor(country))) +
          geom_histogram(alpha = 0.5)
ggplot(dbs, aes(x = testTime, fill = factor(country))) +
          geom_histogram(alpha = 0.5)
```



```{r By-country summary}
countryCounts <- dbs %>% group_by(country) %>%
                        summarise(Samples = n(),
                                  meanTransit = round(mean(transitTime, na.rm = T), 1),
                                  sdTransit = round(sd(transitTime, na.rm = T), 1))
countryCounts <- countryCounts[order(-countryCounts$Samples), ]

# Fix spacing and plot
countryCounts$country <- gsub(" ", "\n", countryCounts$country)
ggplot(countryCounts, aes(x = country, y = Samples)) +
  geom_bar(stat = "identity")
```


```{r By-institution summary}

# Create summary table by institution
institutionCounts <- dbs %>% group_by(institution) %>% summarise(Samples = n())
institutionCounts <- institutionCounts[order(-institutionCounts$Samples), ]

# Fix spacing and plot
institutionCounts$institution <- gsub(" ", "\n", institutionCounts$institution)
ggplot(institutionCounts, aes(x = institution, y = Samples)) +
          geom_bar(stat = "identity")
...
```{r Read geospatial CSV}
latlong <- read.csv("country_latlong.csv", sep = ",", stringsAsFactors = FALSE)
```


```{r Add latlong to existing}
dbs <- inner_join(dbs, latlong, by = "country")
countryCounts <- inner_join(countryCounts, latlong, by = "country") %>% select(-abbr)
```



```{r Base map, a la ggplot}
# Create a base map template, then plot it!
mapWorld <- borders("world", colour="gray50", fill="gray50") # create a layer of borders

ggplot() + mapWorld + geom_point(data = countryCounts,
                                 aes(x = longitude, y = latitude,
                                     color = Samples), size = 2) +
                      theme_bw() + scale_color_gradient(low = "red", high = "green")
```
...
countryCounts <- countryCounts %>%
              mutate(Popup = paste(
                                paste0("<b>", country, "</b>"),
                                paste0("Number of Samples: ", Samples),
                                paste0("Mean Transit Time (days): ", meanTransit),
                          sep = "<br/>"))
                                          
test <- leaflet(countryCounts,
...
```{r Dashboard ticker}
dbs %>% summarise(duration = max(test_date, na.rm = TRUE) -
                    min(spot_date, na.rm = TRUE),
                  nSamples = n(),
                  lastTest = max(dbs$test_date, na.rm = TRUE))
```




```{r Save raw lat/long data}
# Start of geospatial data processing!
latlong <- read.table(text = "AD	42.546245	1.601554	Andorra
                      ...
                      CI	7.539989	-5.54708	Cote dIvoire
                      ...
                      ZW	-19.015438	29.154857	Zimbabwe", sep = "\t", stringsAsFactors = FALSE)
colnames(latlong) <- c("abbr", "latitude", "longitude", "country")
latlong$country <- gsub("Cote dIvoire", "Cote d\'Ivoire", latlong$country)
write.csv(latlong, "country_latlong.csv", row.names = FALSE)
