...
filename <- "2017-8-14_DNMT3a-LNA-ZygProbeTitration"
...
#make tidy RFU table
RFU_spread <- RFU_df %>% 
    select(Sample, Target, Mix, MeanRFU) %>% 
...
# Sort by Mix
RFU_AnnealOrder <- RFU_spread[order(RFU_spread$Sample, RFU_spread$Mix), ]

# Sort Cq by sample as well
# Cq_spread$Sample <- factor(RFU_spread$Sample, levels = row_order, ordered = TRUE)
...
generateDifferences <- function(mixVal) {
  ...
  output <- as.data.frame(t(as.matrix(c(ampMinusNTC, wtMinusNTC,
                                      wtMinusMUT, mutMinusNTC, mutMinusWT))))
  colnames(output) <- c("Amp - NTC", "WT - NTC", "WT - MUT", "MUT - NTC", "MUT - WT")
  if (length(mixColumn) != 0) {
